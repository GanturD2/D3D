...
CGroupTextParseTreeLoader::CGroupTextParseTreeLoader() : m_pRootGroupNode(nullptr), m_dwcurLineIndex(0)
{
}
	...
	for (; m_dwcurLineIndex < m_fileLoader.GetLineCount(); ++m_dwcurLineIndex)
	{
		...
		else if (0 == stTokenVector[0].compare("#--#"))
		{
			for (CTokenVector::size_type i = 1; i < stTokenVector.size(); i++)
			{
				...
				//pGroupNode->m_map_columnNameToIndex.emplace(CMapNameToIndex::value_type(stTokenVector[i], i - 1));
		...
		else
		{
			...
			pGroupNode->m_map_rows.emplace(std::make_pair(key, CGroupNode::CGroupNodeRow(pGroupNode, stTokenVector)));
		}
	}
...
bool CGroupNode::SetChildNode(const char* c_szKey, CGroupNode* pChildGroup)
{
	...
	m_mapChildNodes.emplace(TMapGroup::value_type(c_szKey, pChildGroup));
