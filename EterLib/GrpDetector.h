...
#else
	VOID Build(IDirect3D8 & rkD3D, D3DFORMAT eD3DFmtDefault, uint32_t iD3DAdapterInfo);
#endif
...
#else
	BOOL Build(IDirect3D8 & rkD3D, uint32_t iD3DAdapterInfo, uint32_t iDevType, D3D_CAdapterDisplayModeList & rkD3DADMList, PFNCONFIRMDEVICE pfnConfirmDevice);
#endif
...
#else
	BOOL FindDepthStencilFormat(IDirect3D8 & rkD3D, uint32_t iD3DAdapterInfo, D3DDEVTYPE DeviceType, D3DFORMAT TargetFormat, D3DFORMAT * pDepthStencilFormat);
#endif
...
#else
	BOOL Build(IDirect3D8 & rkD3D, uint32_t iD3DAdapterInfo, PFNCONFIRMDEVICE pfnConfirmDevice);
	VOID GetString(std::string * pstEnumList);

	D3DADAPTER_IDENTIFIER8 & GetIdentifier() { return m_kD3DAdapterIdentifier; }
#endif
