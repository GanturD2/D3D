...
template <class T> struct RangeTester : public SpherePackCallback
{
	...
	void RayTraceCallback(const Vector3d & p1, // source pos of ray
						  const Vector3d & dir, // dest pos of ray
						  float distance, const Vector3d & sect, SpherePack * sphere) override
	{
	...
	void VisibilityCallback(const Frustum & f, SpherePack * sphere, ViewState state) override{};

	void RangeTestCallback(const Vector3d & p, float distance, SpherePack * sphere, ViewState state) override
	{
#ifdef SPHERELIB_STRICT
		if (sphere->IS_SPHERE)
	...
	void PointTest2dCallback(const Vector3d & p, SpherePack * sphere, ViewState state) override
	{
#ifdef SPHERELIB_STRICT
		if (sphere->IS_SPHERE)
...
public:
	...
	void RangeTest(const Vector3d & p, float radius, SpherePackCallback * callback) { m_Factory->RangeTest(p, radius, callback); }

	void PointTest2d(const Vector3d & p, SpherePackCallback * callback) { m_Factory->PointTest2d(p, callback); }
	...
	template <class T> void ForInRange(const Vector3d & p, float radius, T * pFunc)
	{
		RangeTester<T> r(pFunc);
		m_Factory->RangeTest(p, radius, &r /*this*/);
	}

	template <class T> void ForInRay(const Vector3d & p1, const Vector3d & dir, T * pFunc)
	{
		RangeTester<T> r(pFunc);
		/*Vector3d p2;
	...
	template <class T> void ForInRayDistance(const Vector3d & p, const Vector3d & dir, float distance, T * pFunc)
	{
		RangeTester<T> r(pFunc, distance);
		m_Factory->RayTrace(p, dir, &r /*this*/);
	}

	CullingHandle Register(CGraphicObjectInstance * obj);
