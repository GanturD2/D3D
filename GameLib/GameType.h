...
#define MAKE_MOTION_KEY(mode, index) \
	(((uint32_t)(((uint8_t)(mode)) << 24) | ((uint32_t)((uint16_t)(index))) << 8) | ((uint32_t)((uint8_t)(0))))
#define MAKE_RANDOM_MOTION_KEY(mode, index, type) \
	(((uint32_t)(((uint8_t)(mode)) << 24) | ((uint32_t)((uint16_t)(index))) << 8) | ((uint32_t)((uint8_t)(type))))
...
{
	SAttachingData()
		: dwType(0), isAttaching(false), dwAttachingModelIndex(0), pCollisionData(nullptr), pEffectData(nullptr), pObjectData(nullptr)
	{
	}
	uint32_t dwType;
