...
bool CDragonSoulTable::GetDragonSoulGroupName(uint8_t bType, std::string& stGroupName) const
{
...
bool CDragonSoulTable::ReadVnumMapper()
{
	...
	}
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			m_map_name_to_type.emplace(TMapNameToType::value_type(stDragonSoulName, bType));
			m_map_type_to_name.emplace(TMapTypeToName::value_type(bType, stDragonSoulName));
...
bool CDragonSoulTable::ReadBasicApplys()
{
	...
	for (unsigned int i = 0; i < m_vecDragonSoulNames.size(); i++)
	{
		...
		m_map_basic_applys_group.emplace(TMapApplyGroup::value_type(m_map_name_to_type[m_vecDragonSoulNames[i]], vecApplys));
	}
...
bool CDragonSoulTable::ReadAdditionalApplys()
{
	...
	for (size_t i = 0; i < m_vecDragonSoulNames.size(); i++)
	{
		...
		m_map_additional_applys_group.emplace(TMapApplyGroup::value_type(m_map_name_to_type[m_vecDragonSoulNames[i]], vecApplys));
	}
...
bool CDragonSoulTable::CheckWeightTables()
{
	...
	else
	{
		for (size_t i = 0; i < m_vecDragonSoulTypes.size(); i++)
		{
			for (int j = 0; j < DS_GRADE_MAX; j++)
			{
				for (int k = 0; k < DS_STEP_MAX; k++)
				{
					for (int l = 0; l < DS_STRENGTH_MAX; l++)
					{
						float fWeight;
...
bool CDragonSoulTable::CheckRefineGradeTables()
{
	...
	else
	{
		for (size_t i = 0; i < m_vecDragonSoulTypes.size(); i++)
		{
			for (int j = 0; j < DS_GRADE_MAX - 1; j++)
			{
				int need_count, fee;
				...
				if (DS_GRADE_MAX != vec_probs.size())
				{
					TraceError("In %s group of RefineGradeTables, probability list size is not %d.", DS_GRADE_MAX);
					return false;
				}
	...
	else
	{
		for (size_t i = 0; i < m_vecDragonSoulTypes.size(); i++)
		{
			for (int j = 0; j < DS_STEP_MAX - 1; j++)
			{
				...
				if (DS_GRADE_MAX != vec_probs.size())
				{
					TraceError("In %s group of RefineStepTables, probability list size is not %d.", m_vecDragonSoulNames[i].c_str(), DS_GRADE_MAX);
					return false;
				}
				for (size_t k = 0; k < vec_probs.size(); k++)
				{
...
bool CDragonSoulTable::CheckRefineStrengthTables()
{
	...
	for (size_t i = 0; i < m_vecDragonSoulTypes.size(); i++)
	{
		for (int j = CItemData::MATERIAL_DS_REFINE_NORMAL; j <= CItemData::MATERIAL_DS_REFINE_HOLLY; j++)
		{
			int fee;
			float prob;
			for (int k = 0; k < DS_STRENGTH_MAX - 1; k++)
			{
...
bool CDragonSoulTable::CheckDragonSoulExtTables()
{
	...
	for (size_t i = 0; i < m_vecDragonSoulTypes.size(); i++)
	{
		for (int j = 0; j < DS_GRADE_MAX; j++)
		{
			...
			if (0 != by_product && nullptr == CItemManager::Instance().MakeItemData(by_product))
			{
...
bool CDragonSoulTable::GetBasicApplys(uint8_t ds_type, OUT TVecApplys& vec_basic_applys)
{
...
bool CDragonSoulTable::GetAdditionalApplys(uint8_t ds_type, OUT TVecApplys& vec_additional_applys)
{
...
bool CDragonSoulTable::GetApplyNumSettings(uint8_t ds_type, uint8_t grade_idx, OUT int& basis, OUT int& add_min, OUT int& add_max)
{
	if (grade_idx >= DS_GRADE_MAX)
	{
...
bool CDragonSoulTable::GetWeight(uint8_t ds_type, uint8_t grade_idx, uint8_t step_index, uint8_t strength_idx, OUT float& fWeight)
{
	if (grade_idx >= DS_GRADE_MAX || step_index >= DS_STEP_MAX || strength_idx >= DS_STRENGTH_MAX)
	{
...
bool CDragonSoulTable::GetRefineGradeValues(uint8_t ds_type, uint8_t grade_idx, OUT int& need_count, OUT int& fee, OUT std::vector<float>& vec_probs)
{
	if (grade_idx >= DS_GRADE_MAX - 1)
	{
		TraceError("Invalid dragon soul grade_idx(%d).", grade_idx);
		return false;
	}
	...
	vec_probs.resize(DS_GRADE_MAX);
	for (int i = 0; i < DS_GRADE_MAX; i++)
	{
...
bool CDragonSoulTable::GetRefineStepValues(uint8_t ds_type, uint8_t step_idx, OUT int& need_count, OUT int& fee, OUT std::vector<float>& vec_probs)
{
	if (step_idx >= DS_STEP_MAX - 1)
	{
	...
	vec_probs.resize(DS_STEP_MAX);
	for (int i = 0; i < DS_STEP_MAX; i++)
	{
...
bool CDragonSoulTable::GetRefineStrengthValues(uint8_t ds_type, uint8_t material_type, uint8_t strength_idx, OUT int& fee, OUT float& prob)
{
...
bool CDragonSoulTable::GetDragonHeartExtValues(uint8_t ds_type, uint8_t grade_idx, OUT std::vector<float>& vec_chargings, OUT std::vector<float>& vec_probs)
{
	if (grade_idx >= DS_GRADE_MAX)
	{
		TraceError("Invalid dragon soul grade_idx(%d).", grade_idx);
...
bool CDragonSoulTable::GetDragonSoulExtValues(uint8_t ds_type, uint8_t grade_idx, OUT float& prob, OUT uint32_t& by_product)
{
	if (grade_idx >= DS_GRADE_MAX)
	{
		TraceError("Invalid dragon soul grade_idx(%d).", grade_idx);
...
CDragonSoulTable::CDragonSoulTable() : m_pLoader(nullptr)
{
	Clear();
}
