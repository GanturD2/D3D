...
public:
	...
	BOOL GetMotionDataPointer(uint16_t wMotionMode, uint16_t wMotionIndex, uint16_t wMotionSubIndex, CRaceMotionData ** c_ppMotionData);
	BOOL GetMotionDataPointer(uint32_t dwMotionKey, CRaceMotionData ** c_ppMotionData);
	...
	CGraphicThing * NEW_RegisterMotion(CRaceMotionData * pkMotionData, uint16_t wMotionModeIndex, uint16_t wMotionIndex,
									   const char * c_szFileName, uint8_t byPercentage = 100);
	...
	void SetHairSkin(uint32_t eHair, uint32_t ePart, const char * c_szModelFileName, const char * c_szSrcFileName,
