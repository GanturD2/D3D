...
void CActorInstance::SetLoopMotion(uint32_t dwMotion, float fBlendTime, float fSpeedRatio)
{
	...
	{
		Tracenf("CActorInstance::SetLoopMotion(dwMotion=%d, fBlendTime=%f, fSpeedRatio=%f) - GetMotionKey(m_wcurMotionMode=%d, "
				"dwMotion=%d, &MotionKey) ERROR",
...
bool CActorInstance::InterceptMotion(EMotionPushType iMotionType, uint16_t wMotion, float fBlendTime, uint32_t uSkill, float fSpeedRatio)
{
	if (!m_pkCurRaceData)
	{
		Tracef("CActorInstance::InterceptMotion(iMotionType=%d, wMotion=%d, fBlendTime=%f) - m_pkCurRaceData=nullptr", iMotionType, wMotion,
			   fBlendTime);
		return false;
	}
	...
	{
		Tracenf("CActorInstance::InterceptMotion(iLoopType=%d, wMotionMode=%d, wMotion=%d, fBlendTime=%f) - "
				"GetMotionKey(m_wcurMotionMode=%d, wMotion=%d, &MotionKey) ERROR",
...
BOOL CActorInstance::isLock()
{
	...
	switch (dwCurMotionIndex)
	{
		...
		case CRaceMotionData::NAME_SPECIAL_7:
#	ifdef ENABLE_WOLFMAN_CHARACTER
		case CRaceMotionData::NAME_SPECIAL_8:
#	endif
#endif
...
float CActorInstance::GetMotionDuration(uint32_t dwMotionKey)
{
	...
	{
		Tracenf("CActorInstance::GetMotionDuration - Cannot get motion: %d / %d", GET_MOTION_MODE(dwMotionKey),
				GET_MOTION_INDEX(dwMotionKey));
		return 0.0f;
	}
	...
	{
#ifdef _DEBUG
		Tracenf("CActorInstance::GetMotionDuration - Invalid Motion Key : %d, %d, %d", GET_MOTION_MODE(dwMotionKey),
				GET_MOTION_INDEX(dwMotionKey), GET_MOTION_SUB_INDEX(dwMotionKey));
#endif
		return 0.0f;
	}
...
uint32_t CActorInstance::__SetMotion(const SSetMotionData & c_rkSetMotData, uint32_t dwRandMotKey)
{
	...
	if (IsDead())
	{
		if (uNextMot != CRaceMotionData::NAME_DAMAGE_FLYING && uNextMot != CRaceMotionData::NAME_DAMAGE_FLYING_BACK &&
			uNextMot != CRaceMotionData::NAME_DEAD && uNextMot != CRaceMotionData::NAME_DEAD_BACK)
			return 0;
	}
	if (IsUsingSkill())
		__OnStop();
	...
	if (GetEarthQuakeState(GetRace()) != eEarthQuakeType::EARTHQUAKE_TYPE_NONE)
	{
	...
	if (__IsMoveMotion())
	{
		if (uNextMot == CRaceMotionData::NAME_DAMAGE || uNextMot == CRaceMotionData::NAME_DAMAGE_BACK ||
			uNextMot == CRaceMotionData::NAME_DAMAGE_FLYING || uNextMot == CRaceMotionData::NAME_DAMAGE_FLYING_BACK)
		{
			if (!m_isMain)
			{
