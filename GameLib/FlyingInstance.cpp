...
void CFlyingInstance::UpdateAttachInstance()
{
	...
	D3DXQUATERNION q;
	D3DXQuaternionRotationYawPitchRoll(&q, D3DXToRadian(m_pData->m_v3AngVel.y) * CTimer::Instance().GetElapsedSecond(),
	...
	for (it = m_vecAttachEffectInstance.begin(); it != m_vecAttachEffectInstance.end(); ++it)
	{
		...
		switch (rfad.iFlyType)
		{
			...
			case CFlyingData::FLY_ATTACH_TYPE_MULTI_LINE:
				{
					D3DXVECTOR3 p(-sinf(D3DXToRadian(rfad.fRoll)) * rfad.fDistance, 0.0f, -cosf(D3DXToRadian(rfad.fRoll)) * rfad.fDistance);
			...
			case CFlyingData::FLY_ATTACH_TYPE_SINE:
				{
					...
					D3DXVECTOR3 p(-sinf(D3DXToRadian(rfad.fRoll)) * rfad.fAmplitude * sinf(angle), 0.0f,
			...
			case CFlyingData::FLY_ATTACH_TYPE_EXP:
				{
					...
					D3DXVECTOR3 p(-sinf(D3DXToRadian(rfad.fRoll)) * rfad.fAmplitude * exp(-angle) * angle, 0.0f,
...
struct FCheckAnotherMonsterDuringFlying
{
	...
	FCheckAnotherMonsterDuringFlying(const IActorInstance * pOwner, const D3DXVECTOR3 & v1, const D3DXVECTOR3 & v2) : pOwner(pOwner)
	{
		s.fRadius = 10.0f;
		s.v3LastPosition = v1;
		s.v3Position = v2;
	...
	void operator()(CGraphicObjectInstance * p)
	{
		...
		{
			auto * pa = msl::inherit_cast<IActorInstance *>(p);
...
bool CFlyingInstance::Update()
{
	...
	if (m_FlyTarget.IsObject())
	{
		if (!m_bTargetHitted)
		{
			if (square_distance_between_linesegment_and_point(m_v3Position, v3LastPosition, m_FlyTarget.GetFlyTargetPosition()) <
	...
	if (m_pData->m_bHitOnAnotherMonster)
	{
		...
		{
			auto * pHittedInstance = msl::inherit_cast<IActorInstance *>(kCheckAnotherMonsterDuringFlying.GetHittedObject());
...
{
#ifdef ENABLE_GRAPHIC_ON_OFF
	if (m_pOwner2 && !m_pOwner2->IsShowEffects())
		return;
#endif
