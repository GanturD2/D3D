...
void CRaceData::SetHairSkin(uint32_t eHair, uint32_t ePart, const char * c_szModelFileName, const char * c_szSrcFileName,
...
{
	return GetMotionDataPointer(GET_MOTION_MODE(dwMotionKey), GET_MOTION_INDEX(dwMotionKey), GET_MOTION_SUB_INDEX(dwMotionKey),
								c_ppMotionData);
}
...
{
	for (auto & i : m_AttachingDataVector)
	{
		const NRaceData::TAttachingData * pAttachingData = &i;
...
CGraphicThing * CRaceData::NEW_RegisterMotion(CRaceMotionData * pkMotionData, uint16_t wMotionModeIndex, uint16_t wMotionIndex,
											  const char * c_szFileName, uint8_t byPercentage)
{
	auto * pMotionThing = msl::inherit_cast<CGraphicThing *>(CResourceManager::Instance().GetResourcePointer(c_szFileName));
...
{
	auto * pThing = msl::inherit_cast<CGraphicThing *>(CResourceManager::Instance().GetResourcePointer(c_szFileName));
...
bool CRaceData::SetMotionRandomWeight(uint16_t wMotionModeIndex, uint16_t wMotionIndex, uint16_t wMotionSubIndex, uint8_t byPercentage)
{
	...
	{
		TraceError("CRaceData::SetMotionRandomWeight(wMotionModeIndex=%d, wMotionIndex=%d, wMotionSubIndex=%d, byPercentage=%d) - Find "
...
CGraphicThing * CRaceData::GetBaseModelThing()
{
	if (!m_pBaseModelThing)
		m_pBaseModelThing =
			msl::inherit_cast<CGraphicThing *>(CResourceManager::Instance().GetResourcePointer(m_strBaseModelFileName.c_str()));
...
CGraphicThing * CRaceData::GetLODModelThing()
{
	if (!m_pLODModelThing)
	{
		...
		if (CResourceManager::Instance().IsFileExist(strLODFileName.c_str()))
			m_pLODModelThing = msl::inherit_cast<CGraphicThing *>(CResourceManager::Instance().GetResourcePointer(strLODFileName.c_str()));
	}
...
CAttributeData * CRaceData::GetAttributeDataPtr()
{
	...
	return msl::inherit_cast<CAttributeData *>(CResourceManager::Instance().GetResourcePointer(m_strAttributeFileName.c_str()));
}
