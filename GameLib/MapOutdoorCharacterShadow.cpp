...
void CMapOutdoor::CreateCharacterShadowTexture()
{
	...
	// Qualität				Shadow Map Size		GPU-Anforderung		Bildqualität
	// Low-Qualität			128					Sehr niedrig		Sehr grob / unscharf
	// Medium-Qualität		512					Mittel				Akzeptabel
	// High-Qualität		1024				Hoch				Detailliert
	// Ultra-Qualität		2048				Sehr hoch			Sehr scharf
	// Extreme-Qualität		4096				Extrem hoch			Perfekte Schatten (braucht viel VRAM!)

	if (IsLowTextureMemory())
		SetShadowTextureSize(128);	// Low = 128
	//else if (IsHighTextureMemory())
	//	SetShadowTextureSize(1024);  // High = 1024, Ultra = 2048, Extreme = 4096
	//else
	//	SetShadowTextureSize(512);  // Medium = 512
	...
	// D3DFMT_R5G6B5	// 16 Bit, Nicht überall unterstützt, Ohne Alpha-Kanal	-- Standart
	// D3DFMT_A8R8G8B8	// 32 Bit, Wird als Render-Target unterstützt, Mit Alpha-Kanal -- Beste Wahl
	// D3DFMT_X8R8G8B8	// 32 Bit, Wird als Render-Target unterstützt, Ohne Alpha-Kanal -- Zweite Wahl

	if (FAILED(ms_lpd3dDevice->CreateTexture(m_wShadowMapSize, m_wShadowMapSize, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &m_lpCharacterShadowMapTexture, NULL)))
	{
...
bool CMapOutdoor::BeginRenderCharacterShadowToTexture()
{
	...
	{
		TraceError("CMapOutdoor::BeginRenderCharacterShadowToTexture : Unable to Set Shadow Map Render Depth\n");
		bSuccess = false;
	}
#endif _DEBUG

	if (FAILED(ms_lpd3dDevice->Clear(0L, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB(0xff, 0xff, 0xff), 1.0f, 0)))	// White (Hellen) Shadow
	{
#ifdef _DEBUG
